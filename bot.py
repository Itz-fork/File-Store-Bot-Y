...
async def start(bot, cmd):
	...
	if usr_cmd == "/start":
			...
			try:
				...
				if user.status == "kicked":
					await bot.send_message(
						...
						text="Sorry Sir, You are Banned to use me🙄️. You can contact [My Support Group](https://t.me/Nexa_bots) for more info.",
						parse_mode="markdown",
				...
				await bot.send_message(
					chat_id=cmd.from_user.id,
					text="**Sorry Bro! You Must Join My Update Channel To Use Me**\n\nTo Reduce Overload and Free Dyno usage **Only Channel Subscribers Can Use Me**",
					reply_markup=InlineKeyboardMarkup(
						[
							[
								InlineKeyboardButton("🔰️ Join Updates Channel 🔰️", url=invite_link)
							],
							[
				...
				await bot.send_message(
					chat_id=cmd.from_user.id,
					text="Oops! Something went Wrong. Contact my [Support Group](https://t.me/Nexa_bots).",
					parse_mode="markdown",
					disable_web_page_preview=True
				)
				return
				...
				await bot.send_message(
					...
					reply_markup=InlineKeyboardMarkup(
						[
							...
							[
								InlineKeyboardButton("🔄 Refresh / Try Again 🔄", url=f"https://telegram.dog/{BOT_USERNAME}?start=FileStoreBotDBXNB0_{file_id}")
							]
						]
					),
					parse_mode="markdown"
		...
		try:
			...
			await send_stored_file.reply_text(f"**Here is Sharable Link of this file:** https://telegram.dog/{BOT_USERNAME}?start=FileStoreBotDBXNB0_{file_id}\n\n__To Retrive the Stored File, just open the link!__", disable_web_page_preview=True, quote=True)
...
async def main(bot, message):
	if message.chat.type == "private":
		...
		try:
			...
			share_link = f"https://telegram.dog/{BOT_USERNAME}?start=FileStoreBotDBXNB0_{file_er_id}"
			await editable.edit(
				f"**Your File Stored in my Database!**\n\nHere is the Permanent Link of your file: {share_link} \n\nJust Click the link to get your file!",
				parse_mode="Markdown",
				reply_markup=InlineKeyboardMarkup(
					[[InlineKeyboardButton("Open Link", url=share_link)], [InlineKeyboardButton("Bots Channel", url="https://t.me/NexaBotsUpdates"), InlineKeyboardButton("Support Group", url="https://t.me/Nexa_bots")]]
				),
				disable_web_page_preview=True
			)
		except Exception as err:
		...
		if (message.chat.id == Config.LOG_CHANNEL or message.chat.id == Config.UPDATES_CHANNEL):
			return
		else:
			pass
		forwarded_msg = None
	...
	if "aboutbot" in cb_data:
		await cmd.message.edit(
			...
			reply_markup=InlineKeyboardMarkup(
				[
					[
						InlineKeyboardButton("Source Codes of Bot", url="https://github.com/You/will/neve/get/this")
					],
					[
		...
		if Config.UPDATES_CHANNEL:
			...
			try:
				...
				await cmd.message.edit(
					text="**Bro WTF! You Still Didn't Join ☹️, You Must Join My Updates Channel to use Me**\n\nOnly Channel Subscribers Can Use Me",
					reply_markup=InlineKeyboardMarkup(
						[
							[
								InlineKeyboardButton("🔰️ Join Updates Channel 🔰️", url=invite_link)
